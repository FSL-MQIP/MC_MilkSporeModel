...
```{r, eval=FALSE}
#source("Validation/First validation.R", print.eval = T)
```
#Second validation
```{r, eval=FALSE}
source("Validation/Second validation.R", print.eval = T)
```

#Third validation
* Validation with temp shifts
* Raw milk source = NY
* AT distribution = NY
```{r, message=FALSE, warning=FALSE, eval=F}
#third_val_summary = vector()
...
#  return(newLag)
#}
source("Validation/Third validation.R", print.eval = F)
#metrics = c(Tmin, metric_mean, metric_med, metric_sd, metric_D)
#third_val_summary = rbind(third_val_summary, metrics)
#}
#colnames(third_val_summary) = c("Tmin", "metric_mean", "metric_med", "metric_sd", "metric_D")
#third_val_summary
#write.csv(third_val_summary, "performance_summary.csv")
...
spoiled = as.factor(dat2[,ncol(dat2)]>=log10(20000))
...
## Table 3
```{r}
Val2AT = read.csv("InputFiles/LS_ATFreq.csv")
Val2AT$Taxon = Val2AT$Taxon %>% as.factor()
LS_AT_Summary = summary(Val2AT$Taxon)
write.csv(LS_AT_Summary, "LS_AT_Summary.csv")
```
